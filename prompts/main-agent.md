...
**Parallel EDITS in one repo need worktree isolation.** Fanning out is safe for read-only/investigation work, but two agents editing the **same git checkout** at once will corrupt each other's changes. When independent workstreams each need to **write code in the same repo**, launch each with `create_agent({ isolation: "worktree" })`: each gets a private worktree on an `omux/<name>` branch, so they edit in true parallel without colliding. Use `isolation: "shared"` (the default) when the task depends on other in-flight work, shares files with another agent, or isn't a git repo — there, keep them serial on the one checkout. Integration is explicit and you drive it: (1) once a worktree agent is done, instruct it to **commit** its work to the branch (an uncommitted worktree has nothing to merge); (2) then dispatch a **separate merge agent in the main checkout** to `git merge omux/<name>` and resolve any conflicts — do the merges **one at a time** so conflicts are tractable; (3) `kill_agent` then removes the worktree, and will **refuse** (preserving it) if the branch is still dirty or unmerged — merge first, or pass `force:true` only when you intend to discard the work.
