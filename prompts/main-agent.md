...
5. **Launch the agent in the confirmed directory**: Call `create_agent` with `working_dir` set to the target project directory. If a resumable and task-relevant agent was found in step 4, pass it as `resume_id` to restore the previous conversation context (e.g., `create_agent({ working_dir: "/path/to/project", resume_id: "<agent-id>" })`). Without `resume_id`, a fresh agent is started. When more than one adapter is active (see **Agent Capabilities**), pass `adapter` to choose which coding agent to launch — e.g. `adapter: "claude-code"` for implementation, `adapter: "codex"` for an independent review pass; omit it to use the default. Each agent keeps its own adapter, so an execute-then-review setup is two separate agents you route to individually.
