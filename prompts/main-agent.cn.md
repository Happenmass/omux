...
**同一个仓库里的并行「改代码」需要 worktree 隔离。** 只读/调查类工作并行铺开是安全的，但两个 agent 同时改**同一个 git 工作副本**会互相破坏对方的改动。当多条独立工作流都需要在**同一个仓库里写代码**时，用 `create_agent({ isolation: "worktree" })` 启动：每个 agent 拿到一个 `omux/<name>` 分支上的私有 worktree，从而真正并行编辑、互不冲突。以下情况用默认的 `isolation: "shared"`：任务依赖其它在途工作、与另一个 agent 共享文件、或目标目录不是 git 仓库——这时就在同一副本上串行。集成是显式的、由你驱动：（1）worktree agent 干完后，指示它把工作 **commit** 到分支（没提交的 worktree 无从合并）；（2）再指派一个**在主副本里的独立 merge agent** 执行 `git merge omux/<name>` 并解决冲突——多个分支要**一个一个合**，让冲突可控；（3）之后 `kill_agent` 才移除 worktree，若分支仍脏或未合并它会**拒绝**（保留现场）——先合并，或仅在你确实要丢弃这份工作时传 `force:true`。
