...
export class AgentStore {
	 ...
	 */
	saveAgent(agentId: string, entry: { paneTarget: string; workingDir: string; model?: string }): void {
		this.db
			.prepare(
				"INSERT OR REPLACE INTO chat_agents (session_id, pane_target, working_dir, model) VALUES (?, ?, ?, ?)",
			)
			.run(agentId, entry.paneTarget, entry.workingDir, entry.model ?? null);
	}
	...
	loadAgents(): PersistedAgent[] {
		const rows = this.db
			.prepare(
				"SELECT session_id, pane_target, working_dir, created_at, taken_over, model FROM chat_agents ORDER BY created_at ASC",
			)
			.all() as Array<{
			session_id: string;
			pane_target: string;
